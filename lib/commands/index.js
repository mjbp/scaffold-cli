...
const IE11_REPO = 'stormid/scaffold#ie11';
...
module.exports = async function(dest, argv) {
    ...
    let branch = argv['ie11'] ? IE11_REPO : REPO;
    let archive = await gittar.fetch(branch).catch(err => {
    ...
    if (argv['ie11']) info(`-> Adding IE11 support`);
    if (argv['umbraco']) {
        if (argv['umbraco'] === true) info(`-> Adding Umbraco Library`); //no version specified
        else {
	...
	if (pkgFile) {
        ...
        if (argv['umbraco']) pkgData.dependencies = Object.assign({}, pkgData.dependencies, { "@stormid/umbraco-library": argv['umbraco'] === true ? "latest" : argv['umbraco'] });
